...
def parameters():
	...
	parser.add_option("-o","--output-file",action="store",type="string",dest="filename",default="cak_csv.csv",help="Name of output CSV file")
	...
	parser.add_option("-d","--output-directory",action="store",type="string", dest="dir",default=".",help="Path to output directory")
...
def check_records(page_from,pages):
	...
	for page in tqdm(range(page_from,pages + 1)):
		...
		session.evaluate("javascript:__doPostBack('ctl00$mainContent$gridResult','Page$"+str(page+1)+"')",expect_loading = True) #  go to next page
	logger.debug("New links %s" % len(list_of_links))
...
def extract_information(list_of_links):
	...
	if list_of_links is None:
		...
		for html_file in tqdm(list_of_links):
			make_record(make_soup(os.path.join(html_dir_path,html_file["id"])),os.path.join(html_dir_path,html_file["id"]))
